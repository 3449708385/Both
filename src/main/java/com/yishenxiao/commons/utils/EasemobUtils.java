...
import com.yishenxiao.commons.beans.UserPushGroupBean;
import com.yishenxiao.commons.beans.easemob.EasemobFriend;
import com.yishenxiao.commons.beans.easemob.EasemobSignUpBean;
import com.yishenxiao.commons.beans.easemob.EasemobUserSend;
import com.yishenxiao.commons.beans.easemob.ImageMsgContent;
import com.yishenxiao.commons.beans.easemob.SendInfo;
import com.yishenxiao.commons.beans.easemob.SharingExt;
...
import com.yishenxiao.commons.service.impl.easemob.EasemobSendMessage;
...
public class EasemobUtils {
		...
		for(int i=0;i<easemobFriendList.size();i++){
		    ...
		    if(result.toString().contains("error_code")){
	    	   ...
	    	   if(easemobErrorInfo.getError_code()==404){
	    		  logger.error("用户不存在或好友不存在, user: "+easemobFriendList.get(i).getUserName()+" friend: "+easemobFriendList.get(i).getFriendName());
	    	   }else{
	    		  logger.error("添加好友功能出现错误, user: "+easemobFriendList.get(i).getUserName()+" friend: "+easemobFriendList.get(i).getFriendName());
	    	   }
	    	}
	 ...
	 * @return
	 */
	public static List<com.yishenxiao.usermanager.beans.Group> EasemobBatchRegisteredGroups(List<EasemobGroupBean> easemobGroupBeanList){
		List<com.yishenxiao.usermanager.beans.Group> groupList = new ArrayList<com.yishenxiao.usermanager.beans.Group>();
	...
	public String EasemobUploadFile(final String username, final String fileName){		
        try{  	
	        ...
	        try {  
	        	...
	        	logger.error(username+"  "+fileName+"  文件上传失败！");
	        	return "fail";
	        }  
        }catch(Exception e){
        	return "fail";
